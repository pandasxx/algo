...
def get_predictions_result(result_dir, classes):
    ...
    # make dict for classname and filename pair
    for file_name in results_file_list:
        class_name = file_name.split('.')[0]
        dict_class_file[class_name] = file_name
    
    # read every file and store the info
        ...
        for predict_info in all_predict_info:
            ...
            if (len(predict_info_list) == 6):
                ...
                box_info = predict_info_list[1] + " " + predict_info_list[2] + " " + \
                           ...
                           predict_info_list[5]
                pic_box = []
                # add one info to pic box
                pic_box.append(box_info)
                #pic_box.append(box_info)
                
                # add one pic box to pic, one pic can have more than one pic box
...
def get_labels_from_dir(validata_dir, classes):
...
def get_labels_from_file(validata_file, classes):

    file_object = open(validata_file)
    try:
        file_context = file_object.read()
    finally:
        file_object.close()
    file_pathes = file_context.split("\n")
    file_object.close()
    
    validata_file_list = file_pathes
    ...
    for file_name in validata_file_list:
        if (file_name != ""):
            file_path = file_name.split(".")[0] + ".txt"
            with open(file_path) as file_object:
                lebel_info = file_object.read()
            lebel_info = lebel_info.split("\n")[:-1]
            
            pic_name = (file_name.split(".")[0]).split('/')[-1]
            
            dict_pic_label_box_info[pic_name] = lebel_info
...
def cal_evaluate(dict_pic_info, dict_pic_label_box_info, classes, det, iou, img_size):
    ...
    print("du ", all_prediction_object_count)
...
def print_evaluate(classes, all_prediction_object_count, prediction_object_count, label_object_count, recalls, accurates):
    ...
    for i, class_name in enumerate(classes):
        ...
        print ("du ", "%-8s:        %-6s/  %-6s        RECALL: %s"%(class_name, str(prediction_object_count[i]), str(label_object_count[i]), str(recalls[i])))
    ...
    for i, class_name in enumerate(classes):
        print ("du ", "%-8s:        %-6s/  %-6s        ACCURATE: %s"%(class_name, str(prediction_object_count[i]), str(all_prediction_object_count[i]), str(accurates[i])))
#        print("du ", class_name + "\t" + ":" + "\t" + str(prediction_object_count[i]) + "/" + str(all_prediction_object_count[i]) + "      " + "\t" + "acc" + ":" + "\t" + str(accurates[i]))
